...
Int_t test_toyMC_fromPluto(TString infile, Int_t nEvents)
{
    // -----------------------------------------------------------------------
    // define output file and some histograms
    // -----------------------------------------------------------------------
    // set ouput file
    TFile* outfile = new TFile("test_toyMC_fromPluto.root", "recreate");
    ...
    Float_t p2CandTrueP, p2CandRecoP, p2CandTrueTheta, p2CandRecoTheta, p2CandTruePhi, p2CandRecoPhi, 
    ...
    for (Long64_t ev = 0; ev < nEvents; ev++)
    {
        ...
        h_p_1op->Fill(1/p2CandTrueP - 1/p2CandRecoP);
        h_p_tht->Fill(p2CandTrueTheta - p2CandRecoTheta);
        h_p_phi->Fill(p2CandTruePhi - p2CandRecoPhi);
