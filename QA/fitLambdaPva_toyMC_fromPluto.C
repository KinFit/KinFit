...
Int_t fitLambdaPva_toyMC_fromPluto(TString infile, Int_t nEvents)
{
    // -----------------------------------------------------------------------
    // define output file and some histograms
    // -----------------------------------------------------------------------
    // set ouput file
    TFile* outfile = new TFile("fitLambdaPVA_toyMC_myRZ_wrongp.root", "recreate");
    ...
    TH1F* hLambdaMassPostFitPPi = new TH1F("hLambdaMassPostFitPPi", "", 100, 1.070, 1.250);
    hLambdaMassPostFitPPi->SetXTitle(" M_{p#pi^{-}} [GeV/c^{2}]");
    hLambdaMassPostFitPPi->SetYTitle(" events ");
    hLambdaMassPostFitPPi -> SetLineColor(kRed);
    TH1F *hLambdaMassPostFitPPi_probCut = (TH1F*)hLambdaMassPostFitPPi->Clone("hLambdaMassPostFitPPi_probCut");
    hLambdaMassPostFitPPi_probCut -> SetLineColor(kGreen);
   ...
   Float_t p1CandTrueP, p1CandTrueTheta, p1CandTruePhi, p1CandTrueR, p1CandTrueZ, p1CandRecoP, p1CandRecoTheta, p1CandRecoPhi, p1CandRecoR, p1CandRecoZ,
    ...
    for (Long64_t ev = 0; ev < nEvents; ev++)
    {
        ...
        cands1.clear();
        cands1.push_back(proton2_fit);
        cands1.push_back(kaon_fit);
        cands2.clear();
        cands2.push_back(proton1_fit);
        ...
        fitter.addPvaConstraint(lambda_cand);
        if(fitter.fit()){
            ...
            TLorentzVector lambda_fit_ppi = fcand1+fcand2;
            ...
            hLambdaMassPostFitPPi->Fill(lambda_fit_ppi.M());
            ...
            if(fitter.getProb()>0.01){
                ...
                hLambdaMassPostFitPPi_probCut->Fill(lambda_fit_ppi.M());
    ...
    hLambdaMassPostFitPPi->Write();
    hLambdaMassPostFitPPi_probCut->Write();
