...
Int_t fitMissingK_toyMC_fromPluto(TString infile, Int_t nEvents)
{
    // -----------------------------------------------------------------------
    // define output file and some histograms
    // -----------------------------------------------------------------------
    // set ouput file
    TFile* outfile = new TFile("fitMissingK_toyMC_myRZ.root", "recreate");
   ...
   Float_t p1CandTrueP, p1CandTrueTheta, p1CandTruePhi, p1CandTrueR, p1CandTrueZ, p1CandRecoP, p1CandRecoTheta, p1CandRecoPhi, p1CandRecoR, p1CandRecoZ,
    ...
    for (Long64_t ev = 0; ev < nEvents; ev++)
    {
        ...
        TLorentzVector kaon_build = ini - *proton2 - *pion - *proton1;
        ...
        hKMomentumPreFitRes->Fill((1/kaon_build.P()-1/kaon_gen->P())/(1/kaon_gen->P()));
        hKThetaPreFitRes->Fill(kaon_build.Theta()-kaon_gen->Theta());
        hKPhiPreFitRes->Fill(kaon_build.Phi()-kaon_gen->Phi());
        ...
        fitter.addMomConstraint(ini, 0.493677);
        if(fitter.fit()){
            ...
            TLorentzVector kaon_fit = fitter.getMissingDaughter();
            ...
            hKMomPostFit->Fill(kaon_fit.P());
            hKThetaPostFit->Fill(kaon_fit.Theta());
            hKPhiPostFit->Fill(kaon_fit.Phi());
            hKMomPostFitRes->Fill((1/kaon_fit.P()-1/kaon_gen->P())/(1/kaon_gen->P()));
            hKThetaPostFitRes->Fill(kaon_fit.Theta()-kaon_gen->Theta());
            hKPhiPostFitRes->Fill(kaon_fit.Phi()-kaon_gen->Phi());
    ...
    //Outut plots
    outfile->cd();
    plotsStyleMacro(hPChi2, "prob_MissingK");
    plotsStyleMacro(hKMomentumPreFitRes, hKMomPostFitRes, "mom_res_K_miss", "before KinFit", "after KinFit");
    plotsStyleMacro(hKThetaPreFitRes, hKThetaPostFitRes, "tht_res_K_miss", "before KinFit", "after KinFit");
    plotsStyleMacro(hKPhiPreFitRes, hKPhiPostFitRes, "phi_res_K_miss", "before KinFit", "after KinFit");
    plotsStyleMacro(hPiMomentumPreFitRes, hPiMomPostFitRes, "mom_res_Pi_miss", "before KinFit", "after KinFit");
    plotsStyleMacro(hPiThetaPreFitRes, hPiThetaPostFitRes, "tht_res_Pi_miss", "before KinFit", "after KinFit");
    plotsStyleMacro(hPiPhiPreFitRes, hPiPhiPostFitRes, "phi_res_Pi_miss", "before KinFit", "after KinFit");
    plotsStyleMacro_andFit(hPull, "pull_p2_p_miss");
    plotsStyleMacro_andFit(hPull_tht, "pull_p2_tht_miss");
    plotsStyleMacro_andFit(hPull_phi, "pull_p2_phi_miss");
    plotsStyleMacro_andFit(hPull_p_p1, "pull_p1_p_miss");
    plotsStyleMacro_andFit(hPull_tht_p1, "pull_p1_tht_miss");
    plotsStyleMacro_andFit(hPull_phi_p1, "pull_p1_phi_miss");
    plotsStyleMacro_andFit(hPull_p_pi, "pull_pi_p_miss");
    plotsStyleMacro_andFit(hPull_tht_pi, "pull_pi_tht_miss");
    plotsStyleMacro_andFit(hPull_phi_pi, "pull_pi_phi_miss");
