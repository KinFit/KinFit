...
void ToyMC_fromPluto_vtxSmear(TString inFile, Int_t nEvents = 500000)
{
   TFile *outFile = new TFile("toy_montecarlo_rz_vtx_smear.root", "recreate");
   ...
   Double_t smear[6] = {0.025, 0.0009, 0.0009, 0.5, 0.5, 1}; //x 0.1 more realistic, 
   ...
   for(Int_t i=0; i<entries; i++){
      ...
      //Calculate smeared R and Z
      TVector3 p1_mom(p1->Px(), p1->Py(), p1->Pz());
      TVector3 K_mom(K->Px(), K->Py(), K->Pz());
      TVector3 pi_vtx_smear(pi->X() + noise->Gaus(0, smear[3]), pi->Y() + noise->Gaus(0, smear[4]), pi->Z() + noise->Gaus(0, smear[5]));
      TVector3 p1_vtx_smear(noise->Gaus(0, smear[3]), noise->Gaus(0, smear[4]), noise->Gaus(0, smear[5]));
      TVector3 poca_smear_p2 = calcPoca(pi_vtx_smear, p2_mom);
      TVector3 poca_smear_pi = calcPoca(pi_vtx_smear, pi_mom);
      TVector3 poca_smear_p1 = calcPoca(p1_vtx_smear, p1_mom);
      TVector3 poca_smear_K = calcPoca(p1_vtx_smear, K_mom);


      Float_t p1Cand_R = (1. * (poca_smear_p1.X()/cos(p2Cand_phi+M_PI/2)));
      Float_t KCand_R = (1. * (poca_smear_K.X()/cos(p2Cand_phi+M_PI/2)));
      Float_t p2Cand_R = (1. * (poca_smear_p2.X()/cos(p2Cand_phi+M_PI/2)));
      Float_t piCand_R = (1. * (poca_smear_pi.X()/cos(piCand_phi+M_PI/2)));

      Float_t p1Cand_Z = (1. * (poca_smear_p1.Z()));
      Float_t KCand_Z = (1. * (poca_smear_K.Z()));
      Float_t p2Cand_Z = (1. * (poca_smear_p2.Z()));
      Float_t piCand_Z = (1. * (poca_smear_pi.Z()));


      // store in the tree
      // arranged as first all MC values then all reco values for all particles
      Float_t array[49] = {Float_t(p1->P()), Float_t(p1->Theta()), Float_t(p1->Phi()), 0., 0.,  p1Cand_p, p1Cand_theta, p1Cand_phi, p1Cand_R, p1Cand_Z,
                     ...
                     Float_t(p1_vtx_smear.X()), Float_t(p1_vtx_smear.Y()), Float_t(p1_vtx_smear.Z()), Float_t(pi_vtx_smear.X()), Float_t(pi_vtx_smear.Y()), Float_t(pi_vtx_smear.Z())};
